...
# Unblock-File -Path "C:\ruta_donde\se_guardo_el\scriptPS7.ps1"
# .\scriptPS7.ps1 -ExecutionPolicy Bypass
...
if($limpieza -eq 'S' || 's'){
	...
	elseif($nivel -eq 2){
		...
		elseif($errores -eq 'N' || 'n'){
			
		}
		else{
			Write-Host 'Operación Invalida' -ForegroundColor Red
		}
		chkdsk C: /F /R
		...
		elseif($apagarpc -eq 'N' || 'n'){
		
		}
		else{
			Write-Host 'Operacion Invalida' -ForegroundColor Red
		}
